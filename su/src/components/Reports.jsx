...
const Reports = () => {
  ...
  const filterTasks = () => {
    ...
    if (dateFilter) {
      ...
      if (dateFilter === "today") {
        ...
        filteredTasks = filteredTasks.filter(
          (task) => new Date(task.createdAt) >= weekAgo && new Date(task.createdAt) <= now
        );
      } else if (dateFilter === "month") {
        const monthAgo = new Date();
        monthAgo.setMonth(now.getMonth() - 1);
        filteredTasks = filteredTasks.filter(
          (task) => new Date(task.createdAt) >= monthAgo && new Date(task.createdAt) <= now
        );
      }
    }
  ...
  return (
    <div className="marginForleft">
      <div>
        <label htmlFor="projectFilter" className="reports">Filter by Project:</label>
